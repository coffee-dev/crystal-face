...
class ThickThinTime extends Ui.Drawable {
	...
	function drawHoursMinutes(dc) {    		
		...
		var totalDimentions = dc.getTextDimensions(hours + minutes, mHoursFont);  
		var totalWidth = totalDimentions[0];
		var totalHeight = totalDimentions[1];

		y = halfDCHeight - (totalHeight / 2) - (totalHeight / 4);
		...
		if (mVerticalOffset) {
			...
			dc.drawText(
				x,
				y - hoursAscent - (mVerticalOffset / 2),
				mHoursFont,
				hours,
			...
			if (!is24Hour) {
				...
				dc.drawText(
					x,
					y - (hoursAscent / 2) - (mVerticalOffset / 2),
					mSecondsFont,
					amPmText,
		...
		// Horizontal (single-line) layout.
			...
			x += dc.getTextWidthInPixels(time, mHoursFont) / 2;
