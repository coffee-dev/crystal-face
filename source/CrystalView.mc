...
class CrystalView extends Ui.WatchFace {
	...
	function updateGoalMeter(goalType, meter, iconLabel, currentLabel, maxLabel) {
		...
		// Meter.
		meter.setValues(values[:current], values[:max], values[:target], values[:targetMax]);
		...
		if (values[:isValid]) {
			...
			iconLabel.setColor(App.getApp().getProperty("MonoDarkColour"));
		}		
	...
	function getValuesForGoalType(type) {
		...
		var nowSeconds = now.hour * 60 * 60 + now.min * 60 + now.sec;
		...
		switch(type) {
			...
			case :GOAL_TYPE_ACTIVE_MINUTES:
				...
				var nowTime = new Time.Duration(nowSeconds);
				var wakeTime = profile.wakeTime;
				var bedTime = profile.sleepTime;
				var sleepTime = wakeTime.compare(bedTime); 
				if (sleepTime >= 0) {
					maxSeconds -= sleepTime;
					var bedTimeCurrentSeconds = nowTime.compare(bedTime);
					if (bedTimeCurrentSeconds > 0) {
						nowSeconds -= bedTimeCurrentSeconds;
						var wakeTimeCurrentSeconds = nowTime.compare(wakeTime);
						if (wakeTimeCurrentSeconds > 0) {
							nowSeconds += wakeTimeCurrentSeconds;
						}
					}
				}
				else {
					maxSeconds -= (24 * 60 * 60 + sleepTime);
					var bedTimeCurrentSeconds = nowTime.compare(bedTime);
					if (bedTimeCurrentSeconds >= 0) {
						nowSeconds -= (bedTimeCurrentSeconds + wakeTime.value());
					}
					else {
						var wakeTimeCurrentSeconds = nowTime.compare(wakeTime);
						if (wakeTimeCurrentSeconds >= 0) {
							nowSeconds -= wakeTime.value();
						}
						else {
							nowSeconds = 0;  
						}
		...
		values[:target] =  nowSeconds;
		values[:targetMax] = maxSeconds;
	...
	private const BATTERY_FULLY_CHARGED = 80;
	...
	function drawBatteryMeter(dc, x, y, width, height) {
		...
		var fillColour = batteryLevel >= BATTERY_FULLY_CHARGED
			? Helpers.GetHeatMapColour(BATTERY_FULLY_CHARGED, BATTERY_FULLY_CHARGED)
			: Helpers.GetHeatMapColour(batteryLevel, BATTERY_FULLY_CHARGED);
